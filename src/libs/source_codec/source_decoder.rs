...
impl SourceDecoder {
    ...
    pub fn source_decode(&self, encoded_block: Vec<u8>) -> Result<Vec<Frame>, Box<dyn Error>> {
        if encoded_block.len() != (self.block_size_in_bits >> 3) {
        ...
        let mut extractor = BitvecSourceEncodingExtractor::new(self.block_size_in_bits, encoded_block);
        loop {
