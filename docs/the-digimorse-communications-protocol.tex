...
studying LDPC design. A simulation should be done to determine whether the dimensions I have chosen here
are suitable.
...
\begin{itemize}
    ...
    \item $w_{c}$ = column weight; number of 1s present in each column in the parity check matrix
    \item $w_{r}$ = row weight; number of 1s present in each row in the parity check matrix
\end{itemize}

For FT8, this is: $(n=174,k=91)$ so $R={91 \over 174}=0.522988505747126$. There are $174-91=83$ redundant bits.

I had chosen $w_{c}=3$ and $w_{r}=6$ \marginnote{Radford M. Neal's LDPC generator allows choice of $w_{c}$ but doubles this to provide $w_{r}$.}, with $k=126$ as the output from the source encoder.  

After reading \cite{Barry2001}, I realised that for a regular LDPC there is a  constraint on the number of 1s in the parity-check matrix $H$ such that $Mw_{r} = Nw_{c}$ where M is the redundancy and N the codeword length. Initially I chose dimensions that did not honour this constraint. I subsequently chose $k=126$ and doubled this for the codeword size $k=252$, giving a rate of ${1 \over 2}$.

Also, 252 is divisible by 3 and so can be partitioned into groups of 3 bits.

Therefore digimorse uses a (252,126) LDPC, giving 126 bits of redundant parity information.

The LDPC parity check matrix (of order (126,252)) was generated using the LDPC-codes software by Radford M. Neal.
 
\subsection{Doppler Shift protection and Synchronisation}
The resulting 252 bits are partitioned into 3-bit groups, and transformed by the following Gray code, to ensure that
...
8-GFSK modulation? Bandwidth vs transmission speed. The frequency shift between tones is 6.25Hz, the duration of one tone is 0.16s.
