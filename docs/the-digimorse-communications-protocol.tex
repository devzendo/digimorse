...
\newthought{Claude Shannon introduced the following diagram} in his seminal paper "A Mathematical Theory of
    ...
    source, to its eventual destination. The message is transmitted across some medium (the \emph{channel}),
    which inevitably introduces some noise, which means that the received signal is not exactly the same as
    the transmitted signal. It is the task of the transmitter and receiver designer to mitigate the effects of noise
    such that the received message can be delivered to its destination - with any corruption due to its journey
    through the channel removed.
