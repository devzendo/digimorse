...
//    a) Using Radford M. Neal's LDPC-codes with the following command:
//       make-ldpc parity_check_matrix.pchk 126 252 22020 evenboth 3 no4cycle
//                                  rows ___/    \    \____ seed
//                                                \___ cols
//       (The 22020 here is a random seed)
//    b) Using Daniel Estévez's ldpc-toolbox:
//       ldpc-toolbox mackay-neal 126 252 6 3 0 --uniform --min-girth 6 --girth-trials 10000000 --search
// 2) Convert this to alist format file with the command:
//    a) Using LDPC-codes (actually, its transpose):
//       pchk-to-alist -z -t parity_check_matrix.pchk parity_check_matrix.alist
//    b) Using ldpc-toolbox: write the ldpc-toolbox output to parity_check_matrix.alist and convert
//       this back to a .pchk with:
//       alist-to-pchk parity_check_matrix.alist parity_check_matrix.pchk
// 3) Convert the .pchk to a text format file with the LDPC-codes command:
...
//
// Unknowns:
// a) Why does the pchk file generated by LDPC-codes need to be transposed?
// b) Why, when the above has generated Rust, and this is used to create a LinearCode, does the
