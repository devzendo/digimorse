...
%TODO need proper citation for Anderson, painless CRC

The block is then checksummed by a 14-bit CRC - the same as used in WSJT-X's FT8 encoder: using the polynomial 0x2757
and initial value of zero with no reflection\cite{Anderson}. I used this as it is 'good enough' - see the appendices
...
The message size (126 bits) is greater than that in FT8 (91 bits), but I'd like to achieve similar excellent error
...
\begin{itemize}
    ...
    \item $R=k/n, n>k$ =rate of the code
    \item $n-k$ =number of redundant bits
\end{itemize}

For FT8, this is: $(n=174,k=91)$ so $R={91 \over 174}=0.522988505747126 ~ 1/2$. There are $174-91=83$ redundant bits.

Using this rate R in digimorse, ${126 \over n}=0.522988505747126$. So $n=240.923076923077124$ (not an integer). $n$ must
...
$R=k/n = {126 \over 240} = 0.525$. So use a (240,126) LDPC, giving 114 bits of redundancy.

So the 126 bits of payload plus CRC is then augmented with 114 bits of parity information from an (240,126) LDPC code.

The LDPC generator matrix was created using the LDPC-Codes tools from Radford Neal\marginnote{https://github.com/radfordneal/LDPC-codes}.
...
% TODO proper citation for Koopman

There are many possible CRCs, varying in width and choice of polynomial, to yield a specific Hamming Distance.
Some of the most effective ones are evaluated in \cite{Koopman}. However Koopman's research is
...
LDPC. \marginnote{See the discussion on the WSJT-X development mailing list that starts at https://sourceforge.net/p/wsjt/mailman/message/36204768/}
